...
- **HiDPI / 仮想画面 設計大改修（SPRINT-007 / TICKET-017〜020 / DN 反映 v2）**
  - **オフスクリーン物理 px 化（TICKET-018）**: `HspWindow::m_pTargetBitmap` を **物理サイズ**（letterbox 内有効描画領域）で保持する設計に改修。描画コマンド発行時点で `SetDpi(96)` + `SetTransform(Scale(s))` により論理→物理スケール変換を適用。`present()` は SwapChain への **単純転送のみ**（拡縮処理は行わない）。余白はレターボックス / ピラーボックスとして背景塗りとオフセット配置で実現。
  - **`LogicalRenderContext` 中間層追加（TICKET-018）**: `physToLogical` / `logicalToPhys` / `presentMapping` を一元化。`width(clientW, clientH)` / `ginfo_*` / `mouse*` / `pget` / `bmpsave` を論理 px 統一。
  - **テキスト副パイプライン廃止（TICKET-019）**: `mes` / `font` / `sysfont` を他描画 API と同経路に統一。`m_pTargetBitmap`（物理 px）へ直接描画し、DWrite サブピクセル AA が最終物理解像度で動作。HiDPI 環境でテキスト品質が向上（100% DPI 環境では従来と差なし）。
  - **新規命令: `gline_width(float w)`（TICKET-020）**: 線描画の幅指定（論理 px / 既定 1.0）。`w <= 0` は 1.0 にクランプ。仮想 ON / DPI≠96 時は D2D が strokeWidth を `s` 倍に物理化。
  - **新規命令: `gmode_interp(int mode)`（TICKET-020）**: ラスタ画像転送（`picload` / `celput` / `gcopy` / `gzoom`）の補間モード切替（0=NEAREST / 1=LINEAR / 2=ANISOTROPIC、既定 LINEAR）。引数省略時は LINEAR にリセット。
...
- **新規ドキュメント（SPRINT-007 / TICKET-005 / TICKET-022）**
  - `docs/HiDPI.md`（NEW / TICKET-022 で v2 反映改訂）
  - `docs/VirtualScreen.md`（NEW / TICKET-022 で v2 反映改訂）
  - `docs/AnchorLayout.md`（NEW）
  - `docs/MigrationGuide-SPRINT007.md`（NEW / TICKET-022）
- バージョン管理システムの実装
...
- **`vscalemode()` の機能縮退（SPRINT-007 / TICKET-017〜020 / DN 反映 v2）**: present が単純転送になったため、`vscalemode()` の指定は present 経路では作用しない。API 後方互換のため命令自体は残置。新規コードでは `gmode_interp()` の利用を推奨。
- **`gcopy` / `gzoom` の既定補間モード（SPRINT-007 / TICKET-020）**: `mode` 省略時の既定補間モードを **旧 NEAREST → 新 LINEAR** に変更（design v2 §6.8 「既定 LINEAR」方針との一貫性のため意図的変更）。⚠️ HSP3 既存スクリプトで `gcopy` / `gzoom` を `mode` 省略で呼び出していた場合、描画結果が変化する。ピクセルアート利用者は `gmode_interp 0` 明示で従来挙動に復帰可能（詳細は `docs/MigrationGuide-SPRINT007.md` 参照）。
- **`bmpsave` の内部実装（SPRINT-007 / TICKET-018）**: 物理 px の `m_pTargetBitmap` を論理サイズへ D2D `DrawBitmap`（LINEAR）でダウンサンプリングして WIC で BMP 出力する経路に変更。ユーザー IF（論理 px サイズで保存）は維持。
- **`pget` の内部実装（SPRINT-007 / TICKET-018）**: 仮想 ON / DPI≠96 時は「論理 1 px 内に複数の物理 px」が存在するため、中心 1 物理 px の代表値を返す。`pget` → `pset` 往復一致性は近似動作（HSP3 公式と同等）。
...
- ⚠️ **TICKET-017〜020 由来の互換性影響**（詳細は `docs/MigrationGuide-SPRINT007.md` 参照）:
