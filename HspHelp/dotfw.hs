...
p4パラメーターで生成時の動作オプションを指定することができます。以下の値を組み合わせて使用することが可能です。
^p
	オプション値     内容
	-------------------------------------------------------------
	  1              スプライト優先順位を下げる(奥に配置する)
...
^p
	変数名                 内容
	----------------------------------------------
	_dotfw_cursp           プレイヤースプライトNo.
	_dotfw_myx             X座標
	_dotfw_myy             Y座標
	_dotfw_myani           アニメーションカウンター(*)
	_dotfw_mydir           プレイヤーの向き(*)
	_dotfw_myres           プレイヤーの着地情報(*)
	_dotfw_myact           プレイヤーの動作モード(*)
...
%prm
p1,p2,p3,p4,p5
p1(0) : 検索対象の位置(0=下、1=右、2=上、3=左、4=全体)
p2(0) : 検索するセルサイズ
p3(0) : X方向オフセット
p4(0) : Y方向オフセット
p5(0) : 生成オプション値
...
p1,"filename",p2
p1(0) : BG番号(0〜)
"filename" : マップファイル名(*.tmap)
p2(0)  : オプション値
%inst
ドットフレームワーク上で背景として表示するマップを設定します。
p1パラメーターで0から始まるBGの番号を指定します。
"filename"パラメーターでマップファイルを指定します。
マップファイルは、マップエディタ(tamamap)によって作成された.tmapという拡張子が付加されたファイルが対象となります。
マップが設定された後は、df_bgpoint命令で表示位置を調整したり、df_bgview命令でプレイヤーと連動したスクロールを行うことができます。
%href
df_bgpoint
df_bgview
df_bgviewfix
...
p1,p2,p3,p4,p5,p6,p7
p1(0) : BG番号(0〜)
p2(0) : X方向のスクロール開始位置レート(%)
p3(0) : Y方向のスクロール開始位置レート(%)
p4(0) : X方向のスクロール追従レート(%)
p5(0) : Y方向のスクロール追従レート(%)
p6(0) : X方向のスクロール開始位置調整
p7(0) : Y方向のスクロール開始位置調整
%inst
...
背景のスクロールをプレイヤー座標とリンクする場合は、df_bgview命令を使用することができます。
%href
df_loadbgmap
df_bgview
...
p1パラメーターに100を指定した場合、敵とプレイヤーの距離が100(ドット)より小さい場合には弾の発射を行いません。この設定は、すべての敵に共通して使用されます。
%href
df_addenemy
