...
p1,"FileName"
p1         : SHA256値が代入される変数名
"FileName" : SHA256チェックを行なうファイル名

%inst
指定されたファイルのSHA256を求めてp1の変数に代入します。
SHA256値は、文字列型で64文字の16進数として表現されます。
SHA256値は、ファイルの内容を一意に表わすハッシュ値として使用することができます。詳しくは、SHA256についての資料などを参照してください。
%href
varsha256
filemd5
filecrc
...
%inst
指定された変数バッファの内容からSHA256を求めてp1の変数に代入します。
SHA256値は、文字列型で64文字の16進数として表現されます。
SHA256値は、メモリの内容を一意に表わすハッシュ値として使用することができます。詳しくは、SHA256についての資料などを参照してください。
ファイルの内容からSHA256を求める場合は、filesha256命令を使用してください。
%href
filesha256
...
英文字以外の全角を含む文字列をパースする場合は、UTF8形式の文字列としてパースされますので、あらかじめnkfcnv命令等でUTF8形式に変換するようにしてください。
...
オブジェクトの型が文字列だった場合は、そのまま文字列が取得されます。その際に、指定された文字コード変換が行われます。オブジェクトの型が文字列以外だった場合は、型を示す文字列("Number","True","False","Array")が取得されます。
オブジェクトの型が数値の場合は、jsongeti、jsongetd命令を使用してください。
...
"name"で名前を指定した場合、p2で指定されたオブジェクトと同階層にあるオブジェクトから同じ名前を持つオブジェクトを検索し、値を取得します。
値は整数値として取得されます。実数値を取得する場合は、jsongetd命令を使用してください。
オブジェクトの型が文字列の場合は、jsongets命令を使用してください。
指定された名前が存在しない場合は、p1に代入は行なわれずにシステム変数statに1を代入して終了します。
p2を省略またはマイナス値にするとjsonopen命令で読み込ませたデータのルート階層が使用されます。
p2に空(NULL)のJSONポインタ値を指定した場合は、システム変数statが1になり実行は行なわれません。
%href
...
%prm
p1,"name",p2
p1     : 取得された実数値が代入される変数名
"name" : オブジェクト識別名
p2(-1) : JSONポインタ値

%inst
オブジェクトに記録された実数値を取得し、p1で指定した変数に代入します。
p2で対象となるオブジェクト(JSONポインタ値)を指定します。
"name"を省略または中身のない文字列("")に指定した場合は、p2で指定されたオブジェクトそのものに記録されている整数値を取得します。
"name"で名前を指定した場合、p2で指定されたオブジェクトと同階層にあるオブジェクトから同じ名前を持つオブジェクトを検索し、値を取得します。
値は実数値として取得されます。整数値を取得する場合は、jsongeti命令を使用してください。
オブジェクトの型が文字列の場合は、jsongets命令を使用してください。
指定された名前が存在しない場合は、p1に代入は行なわれずにシステム変数statに1を代入して終了します。
p2を省略またはマイナス値にするとjsonopen命令で読み込ませたデータのルート階層が使用されます。
p2に空(NULL)のJSONポインタ値を指定した場合は、システム変数statが1になり実行は行なわれません。
...
%inst
p1で指定されたJSONオブジェクト(JSONポインタ値)に実数パラメーターを設定します。
実数以外のパラメーターを設定する場合は、jsonsetprm命令を使用してください。
既に設定されている値を上書きすることで、構造が壊れる可能性がありますので、設定する場合には注意するようにしてください。
%href
jsonsetprm
jsongetobj
...
実数値を追加する場合は、jsonputd命令をご使用ください。
...
%inst
p1で指定されたJSONオブジェクト(JSONポインタ値)に数値のデータを持つオブジェクトを追加します。
"name"で、追加されるオブジェクトの識別名を、p2で実数値を指定します。
%href
jsonputi
jsonputs
...
--------------------------------------------
    ...
    d         改行をunix改行コード(LF)に変換する
    c         改行をWindows改行コード(CRLF)に変換する
^p
