...
^
HSP3Dish及びHGIMG4ランタイム上では、screen命令で画面を初期化した場合それまでに読み込まれたすべての画像バッファは失われますので注意してください。
...
p5=0〜1(0) : ウィンドウ座標設定オプション
%inst
ウィンドウのクライアントサイズ(実際に表示される大きさ)とウィンドウ表示位置を変更します。
クライアントサイズは、screen命令やbuffer,bgscr命令で初期化された画面サイズより大きくすることはできません。
^
p1,p2パラメータが省略されるか、またはマイナス値の場合は、現在の設定が使われます。
p3,p4パラメーターにより、ディスプレイ上でのウィンドウ座標を設定することができます。
p5の値がデフォルト値か0の場合は、p3,p4パラメーターがマイナス値の場合に現在の設定が使われます。
p5の値が1の場合は、p3,p4パラメーターがマイナス値の場合であっても座標が設定されます。これは、マルチモニタ環境などでマイナス値の座標を扱う必要がある場合などに使用します。
%href
screen
buffer
bgscr
...
^p
 ...
 258 : 加速度センサー値(Z)
      ...
      それぞれの座標軸の加速度を実数(m/s^2)で取得したものになります。(重力加速度9.8m/s^2を含んだ数値が取得されます。)
...
^p
   値 : マクロ名      : 内容
 ------------------------------------------------------------------------
   -1 : celid_auto      未使用のウインドウIDを割り当てる
