...
#const命令の直後に「int」を入れることで、 定義されている数値が強制的に整数と認識されます。
...
例 :
	...
	mes""+dval	; 2が表示される
^p
実数と整数の扱いについては、通常のHSPが持つ計算ルールと異なる点があるのでご注意ください。
...
%inst
スクリプト内で使用する変数を宣言します。
あらかじめ使用する変数を宣言しておくことで、未初期化の変数として検出されることを防ぎ安全に運用することができるようになります。
#var命令では「,」記号で区切ることで、複数の変数を連続で記述することが可能です。
^p
例 :
	#var a,b,c
^p
上の例では、a,b,cという3つの変数をスクリプト内で使用することを宣言しています。
また、型を固定して変数を宣言するための#varint,#varstr,#vardouble,#varlabelも用意されています。
%href
#varint
#varstr
#vardouble
...
var : 宣言する変数名
%inst
スクリプト内で使用する変数を宣言して整数(int)型として固定させます。
...
var : 宣言する変数名
%inst
スクリプト内で使用する変数を宣言して実数(double)型として固定させます。
型が固定された変数は、実行中に異なる型に変更しようとした場合にエラーが発生します。これにより、意図せずに変数の型が変わってしまうことを防ぐことができます。
型の固定を検出するのは、デバッグ実行時のみになります。実行ファイル化したスクリプトや、プラットフォーム変換された環境(iOS,android)では無効になりますので注意してください。
また、あらかじめ使用する変数を宣言しておくことで、未初期化の変数として検出されることを防ぎ安全に運用することができるようになります。
型を固定せずに変数を宣言する場合は、#var命令を使用してください。
...
var : 宣言する変数名
%inst
スクリプト内で使用する変数を宣言してラベル(label)型として固定させます。
型が固定された変数は、実行中に異なる型に変更しようとした場合にエラーが発生します。これにより、意図せずに変数の型が変わってしまうことを防ぐことができます。
型の固定を検出するのは、デバッグ実行時のみになります。実行ファイル化したスクリプトや、プラットフォーム変換された環境(iOS,android)では無効になりますので注意してください。
また、あらかじめ使用する変数を宣言しておくことで、未初期化の変数として検出されることを防ぎ安全に運用することができるようになります。
型を固定せずに変数を宣言する場合は、#var命令を使用してください。
