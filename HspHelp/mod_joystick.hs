...
キーボードから取得された情報は、ポート番号とは関係なく反映されますのでご注意ください。
ジョイスティックの取得などでエラーが発生した場合は、以降の取得がキャンセルされ、再度接続された場合に取得を再開します。
使用する場合は、「#include "mod_joystick2.as"」を記述してください。
(従来の「mod_joystick.as」を指定した場合は、以前の古い仕様で動作し、エラーの復帰などは行われません。)
