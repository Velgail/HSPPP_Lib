...
namespace hsppp {
    ...
    // ============================================================
    // hsppp::String - HSP文字列関数のOOP版を提供する拡張文字列クラス
    // ============================================================
    // std::stringを継承し、HSPの文字列関数をメソッドとして提供
    //
    // 使用例:
    //   import hsppp;
    //   hsppp::String s = "Hello,World,Test";
    //   auto parts = s.split(",");        // {"Hello", "World", "Test"}
    //   s.replace("World", "C++");        // "Hello,C++,Test"
    //   auto sub = s.mid(0, 5);           // "Hello"
    //   auto trimmed = s.trim();          // 両端の空白を除去
    //
    // std::stringとの相互変換:
    ...
    public:
        ...
        String(std::string&& str) noexcept : std::string(std::move(str)) {}
        ...
        /// @brief 文字列を分割（split互換）
        /// @param delimiter 区切り文字列
        /// @return 分割された文字列のベクター
        [[nodiscard]] std::vector<String> split(const std::string& delimiter) const;

        /// @brief 文字列の置換（strrep互換）
        /// @param search 検索文字列
        /// @param replacement 置換文字列
        /// @return 置換回数
        int replace(const std::string& search, const std::string& replacement);

        /// @brief 文字列の一部を取り出す（strmid互換）
        /// @param start 開始インデックス（-1で右から）
        /// @param len 取り出す文字数
        /// @return 取り出した文字列
        [[nodiscard]] String mid(int start, int len) const;

        /// @brief 指定文字を除去（strtrim互換）
        /// @param mode 除去モード（0=両端, 1=左端, 2=右端, 3=全て）
        /// @param charCode 除去する文字コード（デフォルト: 32=スペース）
        /// @return 除去後の文字列
        [[nodiscard]] String trim(int mode = 0, int charCode = 32) const;

        /// @brief 文字列の検索（instr互換）
        /// @param search 検索文字列
        /// @param start 検索開始位置（デフォルト: 0）
        /// @return 見つかった位置（startからの相対位置）、見つからなければ-1
        [[nodiscard]] int indexOf(const std::string& search, int start = 0) const;

        /// @brief パスの一部を取得（getpath互換）
        /// @param type 取得タイプ（1=拡張子除去, 2=拡張子のみ, 8=ディレクトリ除去, 16=小文字化, 32=ディレクトリのみ）
        /// @return 取得した文字列
        [[nodiscard]] String getPath(int type) const;

        /// @brief 文字列長を取得（strlen互換）
        /// @return 文字列長
        [[nodiscard]] int len() const { return static_cast<int>(size()); }
    };
