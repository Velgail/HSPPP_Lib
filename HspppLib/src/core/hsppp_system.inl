...
namespace hsppp {
    ...
    namespace {
        ...
        int peek_impl(const BufferType& buffer, int index) {
        ...
        template<typename BufferType>
        int wpeek_impl(const BufferType& buffer, int index) {
        ...
        template<typename BufferType>
        int lpeek_impl(const BufferType& buffer, int index) {
        ...
        // poke 系のテンプレート実装
        template<typename BufferType>
        void poke_impl(BufferType& buffer, int index, int value, const std::source_location& location) {
            if (index < 0) {
            ...
            buffer[index] = static_cast<CharType>(value & 0xFF);
        }

        template<typename BufferType>
        void wpoke_impl(BufferType& buffer, int index, int value, const std::source_location& location) {
            if (index < 0) {
            ...
            using CharType = typename BufferType::value_type;
            buffer[index] = static_cast<CharType>(value & 0xFF);
            buffer[index + 1] = static_cast<CharType>((value >> 8) & 0xFF);
        }

        template<typename BufferType>
        void lpoke_impl(BufferType& buffer, int index, int value, const std::source_location& location) {
            if (index < 0) {
            ...
            buffer[index] = static_cast<CharType>(value & 0xFF);
            buffer[index + 1] = static_cast<CharType>((value >> 8) & 0xFF);
            buffer[index + 2] = static_cast<CharType>((value >> 16) & 0xFF);
            buffer[index + 3] = static_cast<CharType>((value >> 24) & 0xFF);
        }
    }  // anonymous namespace
