...
A: Visual Studio 2026 (VS 18) 以降が必要です。C++23 のモジュール機能を使用しているため、それ以前のバージョンでは動作しません。
...
A: HSPPP ライブラリが `WinMain` を提供し、ユーザー定義の `hspMain()` を呼び出す設計です。これは HSP の「エントリポイント不要」の体験を再現しています。

<!-- 根拠: WinMain.cpp で extern int hspMain(); を宣言し、hspMain() を呼び出す。戻った後は GetMessage ループで待機。 -->
...
<!-- 根拠: hsppp.ixx のコメント「【設計上の注意】chkbox/combox/listbox には int& 版がありません。安全のため、これらの関数は std::shared_ptr<int> 版のみ提供します。」 -->
...
<!-- 根拠: hsppp.ixx のコメント「input/mesbox は『作成時に値をコピー』する設計のため、参照版でも比較的安全に使用できます（ただし同期は取れません）。」 -->
...
<!-- 根拠: CLAUDE.md の「非安全コーディングの禁止」方針。 -->
...
A: はい、GitHub Issues で受け付けています。HSP の命令で未実装のものがあれば、リクエストしてください。ています。
