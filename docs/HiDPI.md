...
- `mousex()` / `mousey()` はクライアント座標系（仮想画面有効時は論理 px）です。`ginfo_mx` / `ginfo_my` はHSPどおりデスクトップ座標です。
...
| 仮想画面の状態 | 描画コマンドの座標 | `mousex()` / `mousey()` | `ginfo_mx` / `ginfo_my` |
|--------------|---------------------|-------------------------|--------------------------|
| OFF（既定） | 物理クライアント px | 物理クライアント px | デスクトップ座標 |
| ON | 論理 px（`screen()` で指定した `width` / `height`） | 論理クライアント座標 | デスクトップ座標 |
...
ラスタ素材は読み込み時の元解像度で扱われ、拡縮品質はHspppLib拡張の `gmode_interp()` と
`gzoom` のp8指定に依存します。`gzoom` の省略値はHSPどおり0（補間なし）です。高 DPI 環境でドット感のない画像表示が必要な場合は、
