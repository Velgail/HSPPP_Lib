---
layout: default
title: SPRINT-007 移行ガイド（HiDPI / 仮想画面 設計大改修）
---

# SPRINT-007 移行ガイド

HSPPP SPRINT-007 で行われた **HiDPI / 仮想画面 設計大改修** に伴う、後方互換性影響と
...
の新設が言及されていましたが、SPRINT-007 で副パイプラインそのものを廃止し物理 px 経路に
統一したため、本命令は **新設されませんでした**（PM Q-H 完全廃止裁定）。
...
本改修は以下の Sprint 内一次資料に基づいて確定されました（社内参照用）:

- `design-TICKET-017.md` v2（§5.1 / §6.4 / §6.7 / §6.8 / §6.10 / §6.11）
- `audit-TICKET-014.md` §7（既存設計書改訂方針）
- `TICKET-017.md` PM Q-G / Q-H / Q-I / Q-J 裁定
- `TICKET-018.md`（R-A: `LogicalRenderContext` + `presentInternal` 統合 + 論理 px 化）
- `TICKET-019.md`（R-B: テキスト副パイプライン廃止 / `font_mode_buffer` 不採用）
- `TICKET-020.md`（R-C + R-D: 補間モード可変化 + `gline_width` 新設）

ユーザー直送 Director's Notice（2026-05-29T23:55:53+09:00）で示された

> 描画コマンドは論理サイズ座標で発行 / ライブラリは内部のオフスクリーンビットマップに
> **物理サイズで拡縮描画** / `present()` で SwapChain バックバッファへ **単純転送** /
> 余白はレターボックス・ピラーボックス

の方針が、本改修の根拠です。ライブラリ実装本体（`Surface.cpp` / `Window.cpp` /
`hsppp_*.inl` 系）と本ドキュメント群の **両方** が新ポリシーに整合した状態で、本ガイドが
発行されています。Sample のみの修正で済む扱いではありません。
