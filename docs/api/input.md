...
**戻り値:** 現在の描画先ウィンドウのクライアントX座標。仮想画面が有効なら論理座標へ変換されます。
...
**戻り値:** 現在の描画先ウィンドウのクライアントY座標。仮想画面が有効なら論理座標へ変換されます。

`ginfo(ginfo_type_mx)` / `ginfo(ginfo_type_my)` は別物で、HSPと同じデスクトップ座標を返します。
...
座標は現在のウィンドウ内ではなく、仮想デスクトップ上のスクリーン座標です。`x` または `y` を省略した軸は
現在位置を維持します。マルチモニターの負座標へ移動する場合は、負値を非表示指定に使わない `mode=1` または2を指定します。
