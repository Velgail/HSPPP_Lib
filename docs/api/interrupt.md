...
**特徴:**
- **全ウィンドウの描画バッファを自動フラッシュ**してからVSync同期Present
- 戻り値で前回からの経過時間を取得
- フレームドロップ検出が可能
...
**重要:**
- **`vwait()` は描画バッファのフラッシュとVSync同期を自動的に行います**
- 複数ウィンドウがある場合、すべての描画中ウィンドウをフラッシュします
- マルチモニター環境でも機能します

**技術詳細:**
- 内部で全ウィンドウの `isDrawing()` をチェックし、描画中のバッファを `EndDraw()`
- カレントウィンドウで `Present(1, 0)` を呼び出してVSync同期
- ティアリング（画面の横ずれ）を完全に防止
